...
Opening a modal focuses its first focusable element, usually the close button.
...
Provide an accessible name with *aria-labelledby*, referring to the modal title element. If there is no visible title, use *aria-label* on the dialog instead.
